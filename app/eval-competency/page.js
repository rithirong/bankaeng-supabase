...
function EvalCompMain({ session, year }) {
  ...
  function doPrint(){
    ...
    const html = `
      ${makePrintHeader(schoolName, 'รายงานประเมินสมรรถนะสำคัญของผู้เรียน ๕ ประการ', `ชั้น ${cls} ภาคเรียนที่ ${sem} ปีการศึกษา ${year}`)}
      <table><thead>
        <tr><th>#</th><th>รหัส</th><th>ชื่อ-สกุล</th>
        ${ITEMS.map(it=>`<th><div class="vertical-text" style="font-size:9px;">${it.label.split('.').slice(1).join('.').trim()}</div><br/><small>(${MAX_EACH})</small></th>`).join('')}
